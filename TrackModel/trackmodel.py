...
class TrackModel:
    ...
    ## send data to train table in gui
    def set_train_table(self, line, table, t_block):
        b = self.get_train_block_position(line, table, t_block)
        
        block = str(b.get_number())
        section = str(b.get_section())
        line = line.get_name()
        
        table.setItem(0, 0, QTableWidgetItem(line))
        table.setItem(1, 0, QTableWidgetItem(section))
        table.setItem(2, 0, QTableWidgetItem(block))
